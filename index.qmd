...
We measured the voltage at two points in a resistor network to validate the behavior of the circuit. We found the ratio of voltages was: $0.6641\pm 0.0013$ close to the expected value of $0.6667$. We also measured the decay over time of the voltage on a capacitor. We used that to estimate the decay time constant and found that to be $\tau = 74.03 \pm 0.356\mathrm{~ms}$, very close to the expected value of $\tau =66.67\mathrm{~ms}$.
...
Note that these are 5% resistors, we we'd be unsuprised to see a result witha variation of around 5%, or maybe a little more. However the actual result is 11%, which isn't great but acceptable for this lab. 
